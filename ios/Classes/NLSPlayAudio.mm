...
NSInteger lastPackageLength = 0;
static NLSPlayAudio *selfRequest = NULL;
...
-(void)finishFeed:(Boolean)enableFinishCallback {
    if (enableFinishCallback) {
        lastPackageLength = audioDataLength;
        NSLog(@"call finish feed with lastPackageLength %lu",lastPackageLength);
    }
}

-(void)callbackPlayDone{
    [_delegate playDone];
}
...
static void bufferCallback(void *inUserData,AudioQueueRef inAQ,AudioQueueBufferRef buffer) {
    ...
    if (lastPackageLength != 0 && (unsigned int)buffer->mAudioDataByteSize == lastPackageLength) {
        [selfRequest callbackPlayDone];
    }
