...
typedef enum {
    ...
    STATE_START,
    STATE_STOP
}DemoVoiceRecorderState;

NSString * const kDemoVoiceRecorderErrorDomain = @"NlsVoiceRecorderErrorDomain";

#pragma mark - NlsVoiceRecorder Implementation

@interface DemoVoiceRecorder(){
    AudioQueueRef mQueue;
    BOOL _inBackground;
}
@property(atomic,assign) DemoVoiceRecorderState state;
...
@implementation DemoVoiceRecorder
    ...
    if (mQueue) {
        ...
        if(skipFrame++ == 3){
            ...
            if (status == noErr) {
                ...
                [_delegate voiceVolume:_currentVoiceVolume];
            }
        }
    }
}
...
static void inputBufferHandler(void *                          inUserData,
    ...
    @autoreleasepool {
        ...
        DemoVoiceRecorder *recorder = (__bridge DemoVoiceRecorder*) inUserData;
...
-(void) _disposeAudioQueue{
    ...
    @try {
        ...
        NSLog(@"throw error,when AudioQueueDisposed!");
    }
